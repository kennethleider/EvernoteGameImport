...
object Game {
  ...
  def load(key : String, userID : String) = {
    val cacheFile = new File(cacheDir, s"user-${userID}.xml")
    if (!cacheFile.exists() || Instant.ofEpochMilli(cacheFile.lastModified()).compareTo(Instant.now().minus(2, ChronoUnit.DAYS)) != 1) {
      val url = s"http://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key=${key}&steamid=${userID}&format=xml&include_played_free_games=1&include_appinfo=1"
    ...
    val root = XML.loadFile(cacheFile)
    val games = (root \ "games" \ "message").map { message =>
      val id = (message \ "appid").text
      val playtime = (message \ "playtime_forever").text
      Game(id, playtime.toInt)
    }.filterNot { game =>
      addOns.contains(game.appID)
    }.filterNot { game =>
    ...
    (keep ++ newGames).map { game =>
      val extraTime = timeUpdates.filter { _.appID.equals(game.appID)}.map { _.playtimeMinutes }.sum
